...
\begin{itemize}
  \item Generalization of containers and W-types
  \item MLTT + funExt
  \item Agda formalization
\end{itemize}
...
\textbf{Containers} are polynomial functors:
\[ X \mapsto \sum_{s : S} P(s) \to X \]
\end{block}
...
\begin{block}{Definition}
\textbf{W-types} are the initial algebras of containers.
They correspond to all (strictly positive) inductive types.
\end{block}
...
\begin{block}{Definition}
\textbf{2nd-order functors} have all following constructions
\end{block}
...
It is a functor of \textbf{[Set,Set]}:
...
\begin{block}{Definition}
\textbf{2nd-order containers} are polynomial 2nd-order functors (with applications)
\end{block}
...
Functor of \textbf{[Set,Set]} interpretation:

\begin{exampleblock}{}
\begin{verbatim}
2⟦_⟧F : 2Cont → Func → Func
2⟦_⟧F₁ : (H : 2Cont) {𝔽 𝔾 : Func} → NatTrans 𝔽 𝔾 → NatTrans (H 𝔽) (H 𝔾)
\end{verbatim}
\end{exampleblock}

\pause
Functor of \textbf{Cont} interpretation:
...
\begin{block}{Definition}
\textbf{2nd-order W-types} are the initial algebras of 2nd-order containers. They correspond to all (strictly positive) inductive types with kinds \texttt{(Set → Set)}.
\end{block}
...
\begin{exampleblock}{}
  \begin{columns}
    ...
    \begin{column}{0.55\textwidth}
      \begin{tikzcd}[column sep=huge]
        \texttt{app H (2W H)} \arrow[r , "\texttt{app₁ H (fold2W α)}"] \arrow[d , "\texttt{2sup}"] & \texttt{app H SP} \arrow[d, "\texttt{α}"] \\
        \texttt{2W H} \arrow[r , "\texttt{fold2W α}"] & \texttt{SP}
      \end{tikzcd}
    \end{column}
  \end{columns}
\end{exampleblock}
...
Their signatures are in even higher kinds:
...
\begin{frame}[fragile]{Higher-Order Functors}

\begin{block}{Definition}
\textbf{Higher-order functors} are functions with \textbf{isHFunc}, and they are mutually defined with \textbf{higher-order categories}
\end{block}
...
Higher-order categories are not iterated functor categories!
\end{alertblock}

\end{frame}

\begin{frame}[fragile]{Higher-Order Containers}
...
\begin{itemize}
  \item hereditary substitution:\\
    \begin{center}
    Recursive substitutions on structures
    \end{center}
...
\begin{block}{Definition}
\textbf{Higher-order containers} are closed normal forms
\end{block}
...
\begin{frame}[fragile]{Higher-Order Containers to Functors}
...
\pause
Initial algebras of endo-higher-order containers:
...
\begin{tabular}{|c|c|}
  \hline
  \texttt{ℕ : Set} & W on Cont \\
  \hline
  \texttt{ℕ∞ : Set} & M on Cont \\
  \hline
  \texttt{Fin : ℕ → Set} & IW on ICont \\
  \hline
  \texttt{Fin∞ : ℕ → Set} & IM on ICont \\
  \hline
  \texttt{List, Bush : Set → Set} & 2W(\checkmark) on 2Cont(\checkmark) \\
  \hline
  \texttt{Stream : Set → Set} & 2M(?) on 2Cont \\
  \hline
  \texttt{MaybeT : (Set → Set) → Set → Set} & HW(?) on HCont(\checkmark) \\
  \hline
  \texttt{Vec : Set → ℕ → Set} & (?) on (?) \\
  \hline
  ... & ... \\
\end{tabular}
